...
class DevCapabilityActions(private val activity: MainActivity) {
   ...
   */
  fun openWithSkillGroup() {
    if (!ChannelSetup.ensureReady(activity)) return
    askSkillGroup("打开客服并指定技能组", "填你工作台里的技能组名称") { group ->
   ...
   */
  fun switchSkillGroup() {
    askSkillGroup("切换技能组", "留空 = 清除指派,回到默认分派") { group ->
      if (group.isEmpty()) {
        HecongChat.setRouting(null as String?)
        toast("已清除技能组指派")
      } else {
        HecongChat.setRouting(group)
        toast("已指派到「$group」—— 下一次新对话生效")
      }
    }
  }
  ...
  fun demoOrderPicker() {
    ...
    // 同上:数据在点击回调里现给
    HecongChatActivity.start(activity, DemoConfig.buildChatConfig(activity))
    toast("已加「订单」入口 —— 在输入框正上方")
  }

  /**
   * **两个位置对比**:同一个按钮放附件面板 vs 放快捷区,一眼看出差别。
   * 附件面板 = 收着(点 + 号才看到);快捷区 = 显眼(输入框正上方)。
   */
  fun demoBothSlots() {
    if (!ChannelSetup.ensureReady(activity)) return
    HecongChat.registerAction("demo-attach", "我在附件面板", "attach")
    HecongChat.registerAction("demo-quick", "我在快捷区", "quick")
    HecongChatActivity.start(activity, DemoConfig.buildChatConfig(activity))
    toast("两个位置各放了一个按钮,进去对比看")
  }

  /** 撤掉本页注册过的所有自定义按钮(演示 `unregisterAction`;同 id 再注册 = 覆盖) */
  fun clearActions() {
    listOf(ACTION_PRODUCT, ACTION_ORDER, "demo-attach", "demo-quick")
      .forEach { HecongChat.unregisterAction(it) }
    toast("已撤掉所有自定义按钮")
  }
  ...
  private fun askSkillGroup(title: String, hint: String, onOk: (String) -> Unit) {
